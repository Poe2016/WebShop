...
String detail = request.getParameter("detail");//小类
ArrayList<Good> goodslist=(ArrayList<Good>)session.getAttribute("goodslist");//所有商品列表
ArrayList<Good> showlist=new ArrayList<Good>();//显示商品列表
if(detail == null){//只有大类
	title = type;
	for(Good g:goodslist){
		if(g.getType() != null && g.getType().equals(type)){
			showlist.add(g);
		}
	}
}else{
	title = type+"-"+detail;
	for(Good g:goodslist){
		if(g.getType() != null && g.getDetil_type()!= null && g.getType().equals(type) && g.getDetil_type().equals(detail)){
			showlist.add(g);
		}
	}
}
