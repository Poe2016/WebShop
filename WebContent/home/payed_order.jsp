...
<%	String longinmesg="你好，请登录";
	...
	String sql = "select count(*) onum from order_t where mail=\'"+mail+"\' and pay=0";//查询待付款订单
	try{
	...
	sql = "select count(*) rec_num from order_t where mail=\'"+mail+"\' and pay=1 and receive_state=0";//查询待收货订单
	try{
		rs = st.executeQuery(sql);
		if(rs.next()){
...
<body>
	...
	<dl class="fore1">
		...
		<dd class="fore1_4 curr" id="_MYJD_yushou">
	...
	<dl class="fore3">
		...
		<dd class="fore3_1" id="_MYJD_cashbox">
			<a href="send_received.jsp" target="_self">已收货订单</a>
		</dd>
		<dd class="fore3_1" id="_MYJD_cashbox">
				...
				<div id="main">
				 	<div id="order02" class="mod-main mod-comm lefta-box">
				 		<div class="mt">
	                       <ul class="extra-l">
	                           <li class="fore1">
	                           	<a href="buyer_order.jsp" id="ordertoPay"  class="txt">待付款</a>
	                           	<a href="buyer_order.jsp"><em><%=onum %></em></a>
	                           </li>
	                           <li>
	                           	<a href="payed_order.jsp" id="ordertoReceive" class="txt curr">待收货</a>
	                           	<a href="payed_order.jsp"><em><%=rec_num %></em></a>
	                           </li>
                      	...
                      	<div class="mc">
                      		<table class="td-void order-tb">
                                	...
                                	<tr>
                                    	<th>
                                    		<div class="ordertime-cont">
												<div class="time-txt">所有待收货订单<b></b>
                            	...
                            	<%
                            		...
                            		sql = "select * from order_t where mail=\'"+mail+"\' and pay=1 and receive_state=0";//查询待付款订单
                            		try{
                            			rs = st.executeQuery(sql);
                            			while(rs.next()){
                            				int ord_id = rs.getInt("ord_id");
                            				String goodno = rs.getString("goodno");//商品编号
                            				String date = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(rs.getDate("ord_date"));
                            				int ord_num = rs.getInt("ord_num");//数量
                            				int money = rs.getInt("money");//总价
                            				String receiver = rs.getString("receiver");//收货人
                            				String address = rs.getString("ord_address");//收货地址
                            				Good g_ord = null;
                            				//获取该商品
								        ...
								        <tr class="tr-bd" id="track<%=ord_id %>" >
								            ...
								            <td rowspan="1" id="operate<%=ord_id%>">
								                <div class="operate">
								                    ...
								                    <br>
								                    <a href="" class="btn-again btn-again-show" target="_blank">
								                    	<b></b>
														查看物流
													</a>
													
													<a onclick="confirm_receive(<%=ord_id %>)" class="btn-again btn-again-show" target="_blank">
								                    	<b></b>
														确认收货
													</a>
													<br>
